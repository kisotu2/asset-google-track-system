...
if(!$inside){$message='Latest authorised check-in is outside an approved area.';$a=$conn->prepare('INSERT INTO location_alerts(laptop_id,location_id,message) VALUES(?,?,?)');$a->bind_param('iis',$asset,$locationId,$message);$a->execute();}
audit($conn,'location_checkin','device_location',$locationId,['asset_id'=>$asset,'source'=>'browser']);echo json_encode(['message'=>$inside?'Check-in recorded successfully.':'Check-in recorded; an approved-area alert was created.']);
